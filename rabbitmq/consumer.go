...
var (
	errHandlerNameBusy   = errors.New("handler with this name is already registered")
	errChannelNotCreated = errors.New("channel is not created")
)

// ConsumerHandle handles a received message.
...
// It declares a queue and binds it to an exchange when one is provided.
...
func (c *Consumer) handle(msg amqp.Delivery) {
	...
	if err := msg.Nack(false, false); err != nil {
		log.Error().Err(err).Msg("could not negatively acknowledge message")
	}
}
